...
3. ~~**H2 — email-only login is not tenant-scoped.**~~ **RESOLVED** — email is now unique platform-wide (`uq_users_email_active`), so the email-only lookup resolves to exactly one account. See §6.2.
...
| 3 | ~~**High**~~ **Resolved** | Auth / Tenancy | H2: email-only login lookup not tenant-scoped | `AuthServiceImpl.java:141`, `User.java:14-26`, `db/indexes.sql` | Cross-tenant email reuse → 500 | Made email unique platform-wide (`uq_users_email_active`) instead of disambiguating at login — the lookup is now well-posed. See §6.2 |
...
### 6.2 H2 — email-only login → cross-tenant collision → **RESOLVED**
- ~~`AuthServiceImpl.userLogin` calls `userRepository.findByEmailAndDeletedAtIsNull(email)` with **no tenant**~~; ~~the schema's uniqueness is `(email, tenant_id)`, so the same email **can** exist in multiple tenants~~.
- **Fixed by moving the schema, not the login.** Email is now unique platform-wide — `uq_users_email_active` (`db/indexes.sql`, partial on `deleted_at IS NULL`); `uq_user_email_tenant` dropped. The email-only lookup is now correct: an address resolves to exactly one account, and the tenant is a *result* of the lookup rather than an input to it. This is the **opposite** of the disambiguation fix this document originally prescribed — see `backend-review-findings.md` H2 for why (`organizationCode` is not exposed to tenant users, so a login field was unshippable).
- Verified in practice: the bug reproduced live on 2026-07-16 as `NonUniqueResultException: 2 results were returned` when `DevDataSeeder` seeded `tenant_admin@demo.crm` into a second tenant. The seeder's `getOrCreateTenant()` used an unordered `findAll().findFirst()` and attached to a tenant it did not own — fixed to resolve by code.
- Cost accepted: one person cannot hold accounts in two tenants under the same address.
...
4. ~~**H2 tenant-disambiguated login** (Finding #3) — fail-closed on multiple email matches; introduce a tenant discriminator (subdomain/selector).~~ **DONE, differently** — email made unique platform-wide, so there is nothing to disambiguate. See §6.2.
