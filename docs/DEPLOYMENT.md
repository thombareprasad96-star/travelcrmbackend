...
| `V2__lead_code.sql` | **Twelve parts.** **1** — `leads.lead_code` + `lead_sequences` + backfill + counter seed + indexes. **2** — username login: `users.username` (+ backfill, NOT NULL, uniqueness swapped off `email`) and `activity_logs.username`. **3** — `booking_expenses`, the per-booking expense ledger. **4** — booking financial engine: `cost_type`, `total_internal_costs`, `sunk_internal_costs`. **5** — per-tenant tax configuration (booking GST/TCS, TDS sections, `overseas_tour_package`). **6** — fleet phase 0: `tenants.timezone` + the `FLEET_MONEY_READ` permission backfill. **7** — fleet ledger: `fleet_trip_legs`, `fleet_expenses`, `fleet_cash_entries`, `fleet_trip_settlements`, `fleet_period_closes` + trip fx columns. **8** — `fleet_allowance_policies`. **9** — `fleet_compliance_documents` + backfill of the legacy vehicle/driver expiry columns. **10** — one document vocabulary: `fleet_document_alerts.doc_type` widened + RC/PERMIT/DRIVER_LICENSE migrated, `document_id` added. **11** — `fleet_attachments` (bytea receipts/scans/signed sheets; counts toward the storage quota). **12** — `fleet_parties` (standalone owner directory) + the `FLEET` plan code widened across **all seven** CHECK constraints that name the plan enum (`plans`, `tenants`, `subscriptions`, `billing_records` ×2, `upgrade_requests` ×2) — miss one and provisioning fails at whichever step first writes the new value. |
