...
public class DestinationController {
    ...
    // ── POST image upload ─────────────────────────────────────────────────────
    // Mirrors Hotel/Sightseeing/Vehicle: file goes through the backend so the tenant
    // storage quota is enforced and bytes are metered; caller sends the returned URL
    // back as `imagePath` on create/update.

    @PostMapping(value = "/destinations/upload-image", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @PreAuthorize("hasAnyAuthority('PLATFORM_ADMIN', 'MASTER_MANAGE')")
    public ResponseEntity<ApiResponse<Map<String, String>>> uploadImage(
            @RequestParam("file") MultipartFile file) {
        String url = destinationService.uploadImage(file);
