...
-- ── Quick Quote room planning / Hotel Master rates ─────────────────────────
-- Mirrors V4__quick_quote_room_snapshots.sql for SQL_INIT_MODE=always environments. Hibernate is
-- validate-only, so neither the rate column nor the two optional room-detail tables can be left to
...
CREATE INDEX IF NOT EXISTS idx_lead_room_allocation_lead ON lead_room_allocations (lead_id);

ALTER TABLE IF EXISTS lead_room_allocations DROP CONSTRAINT IF EXISTS fk_lead_room_allocation_lead;
ALTER TABLE IF EXISTS lead_room_allocations ADD CONSTRAINT fk_lead_room_allocation_lead
...
-- The incoming-leads feed: "every inbound lead in this tenant still open to claim, newest first".
-- Recreate rather than CREATE IF NOT EXISTS because the predicate gained the origin term after the
-- index first shipped; IF NOT EXISTS would silently keep the broader, stale index forever.
